  ...
  // ===================================================================
  {
    ...
    body: (
      <>
        ...
        <P>
          A common unit raises a question it does not itself answer. Dollars in Pockets tells you <I>how much</I>. Whether you can stand behind that much is a second question, and it is the one that usually goes ungraded.
        </P>
        <P>
        ...
        <P>
          The scoring rule is what separates this from a maturity score. The verdict is the <B>minimum</B> of the four. One red link caps the claim no matter how strong the other three.
        </P>
        <P>
        ...
        <P>
          The analogy has a limit worth stating, because it weakens the rule proposed here. GRADE&rsquo;s outcomes are entangled: mortality, hospitalization and adverse events all come from one intervention given to one set of patients, so weak evidence on one constrains what can be said about the balance. Impact pathways are separable. Whether legal fees were avoided for one group does not depend on whether housing improved for another. Taking the minimum across them is a harsher rule than GRADE&rsquo;s, and a fund is entitled to say so.
        </P>
        <P>
        ...
        <P>
          GRADE&rsquo;s other half answers the question a program officer raises next. Low certainty does not forbid action. GRADE allows a strong recommendation on low-certainty evidence in five defined situations, though it treats these as rare, and audits find guideline panels reach for them more often than the guidance sanctions (<a href="https://pubmed.ncbi.nlm.nih.gov/25618534/" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">Alexander et al., <I>J Clin Epidemiol</I>, 2016</a>). Certainty governs what a fund may claim. It leaves the funding decision open. A pathway that earns only <I>supports</I> can still be worth funding, and both sentences can be said out loud without contradiction.
        </P>
        <P>
          The responsible-AI frameworks a fund is likeliest to reach for do not close this gap either. The National Institute of Standards and Technology (NIST)&rsquo;s <a href="https://airc.nist.gov/airmf-resources/airmf/3-sec-characteristics/" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">AI Risk Management Framework</a> (2023) names validity and reliability as the first characteristic of a trustworthy system, and the <a href="https://eur-lex.europa.eu/eli/reg/2024/1689/oj" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">EU AI Act</a> (2024) sorts systems into risk tiers with obligations scaled to the tier. Neither treats validity in the argument-based sense meant here. NIST does not define validation itself. It adopts the ISO 9000:2015 definition: confirmation that the requirements for a specific intended use have been fulfilled. That is a question about whether a system performs as specified. NIST is revising that framework, and no revised version has been published. Argument-based validity asks whether a reported figure can carry the decision resting on it. <InternalLink slug="what-is-this-system-measuring">The same absence shows up in higher education</InternalLink>, wherever a number is asked to justify a decision.
        </P>
  ...
  // ===================================================================
  {
    ...
    body: (
      <>
        ...
        <P>
          These questions map almost exactly to the responsible-AI cluster appearing across the <a href="https://www.nist.gov/itl/ai-risk-management-framework" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">NIST AI Risk Management Framework (RMF)</a>, the <a href="https://artificialintelligenceact.eu/" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">EU AI Act</a> risk tiers, the NAM Code of Conduct, APA&rsquo;s ethical guidance, the WHO workshop outputs, and the industry frameworks circulating alongside them. They are the right floor for responsible AI in mental health. The institution that can answer them is operating at a much higher standard than the institution that has not asked them at all.
        </P>
        ...
        <P>
          Not for lack of response. RDoC has generated fifteen years of dimensional-construct research, <a href="https://hitop.unt.edu/" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">HiTOP</a> has proposed an empirically-driven alternative taxonomy, DSM-5-TR has folded in dimensional criteria alongside categorical, and precision psychiatry and digital phenotyping are actively building next-generation measurement infrastructure. Those are extensions of the measurement framework around the constructs. They are not resolutions of the constructs themselves, or of construct drift as those constructs are applied &mdash; the level at which consumer-scale AI mental-health tools now make their claims.
        </P>
        <P>
  ...
  // ===================================================================
  {
    ...
    body: (
      <>
        ...
        <P>
          A rejection is a binary outcome produced by a system with many causes. At least seven distinct states sit behind it. The role was canceled. An internal candidate existed. The budget froze. The requirements changed. The screen misread the document. A stronger candidate applied. The person could not do the work.
        </P>
        <P>
        ...
        <P>
          Note what that rules out. The employer cannot label its own rejections as errors, because labeling them would require the observation the screen prevented. A bad hire eventually becomes observable. A rejected candidate who would have succeeded does not. One of the two errors can be managed and the other cannot be seen.
        </P>
        ...
        <P>
          The historical defense of the one-line rejection was cost. At two thousand applications, writing individual feedback was not possible, and nobody pretended otherwise. That defense has expired. The assessment work is already done and the text is close to free to produce. What is missing is any obligation to send it.
        </P>
        <P>
        ...
        <P>
          And the return has to come second. Feedback trains behavior. If the screen is reading r&eacute;sum&eacute;-writing skill and title adjacency, then telling candidates what it read teaches them to optimize those, and every future application converges on whatever the instrument rewards. The signal degrades. A reason is worth returning only when the instrument is measuring something worth optimizing toward. Version the construct first. Returning a reason drawn from an unspecified construct broadcasts noise with the employer&rsquo;s authority behind it.
        </P>
        <P>
  ...
  // ===================================================================
  {
    ...
    body: (
      <>
        ...
        <P>
          I ran data for a K-8 charter network in the Bronx, five schools, about 1,800 children. Two of the districts we had moved into were sixteen to seventeen percent English learners. We were serving seven or eight. Whether we were reaching the children who needed us, and whether that was changing, was the question I could not answer.
        </P>
        <P>
  ...
  // ===================================================================
  {
    ...
    body: (
      <>
        ...
        <P>
          Twenty-three of the sixty-five name a decision the hire may make, anywhere in the document. Thirty-five percent. Two coders worked from the same evidence packet, each seeing the rule and the text but neither seeing the other&rsquo;s codes or the postings themselves. They agreed on ninety-four percent of judgments. Then a separate pass swept all sixty-six for the verb patterns the adjudication had settled on, and it turned up five postings both coders had missed. Agreement tells you two readers were consistent. It cannot tell you they were right, and here they were consistently wrong five times.
        </P>
        <P>
          Seven name a measure of the role&rsquo;s own work, which is eleven percent, and it is <InternalLink slug="what-is-this-system-measuring">the same gap I went after in the evaluation of AI systems</InternalLink>, arriving here one document earlier and about a person instead. Inside those seven there are thirty-three enumerated success items, and exactly one carries a number: a target net promoter score, above 0.70, at a consultancy. That is the only threshold applied to a post-holder&rsquo;s own work anywhere in the corpus. The other thirty-two name a condition without saying how anyone would read it. Stakeholders trust the numbers. Dashboards are widely adopted. Time-to-insight comes down. How widely, by when, down to what, measured by whom &mdash; none of that is written, so none of it could be missed.
        </P>
        <P>
          Read down the three and the shape is a descent.
        </P>
        ...
        <P>
          Counted the other way, by how many anchors a single document founds rather than by how many documents found each anchor: four postings of sixty-five have all three. Twenty-one have two, forty have one, and none have zero.
        </P>
        ...
        <P>
          Forty-two of the sixty-five name what the person must bring and never say what the role may decide. Not in a scope paragraph, not in responsibilities, not anywhere. These are director-level roles and above. From here the argument needs an assumption the corpus does not measure: that at this level, settling things is most of the work. Grant that, and those items are being asserted as necessary for a job the document has not stated.
        </P>
        <P>
          That is a claim about the document and it stops there. A job analysis may well exist somewhere &mdash; in a competency model, in a hiring manager&rsquo;s head, in the conversation that produced the requisition. Nothing here says otherwise. What the corpus supports is narrower: these documents specify the candidate and mostly leave the role&rsquo;s decision scope unstated. There is a second question underneath it that I did not code at all: whether, for any given requirement, the document says what work makes that attribute necessary. Searching for a named decision is not the same as checking eight hundred and thirteen items for a stated warrant, and only the first was done here.
        </P>
        <P>
          Before any of this was a corpus, I sat with one job description and counted its and/or constructions. There were four. Every one of them sat at the point where the document had to say whether the role decided something or assisted with it. The slash is the document arriving at the question and declining to answer it. One document proves nothing and I have not counted this marker across the sixty-five. It did change what I thought I was reading. I had been treating these as loosely written documents. After the and/or count I started reading them as documents that arrive at the decision question and step around it.
        </P>
        <P>
        ...
        <P>
          Sixty-four postings carry a requirements section. Across them there are eight hundred and thirteen items &mdash; one bullet or one sentence apiece &mdash; and <B>not one of those items names a decision the hire may make.</B> Not in the required block, not in the preferred block. This is a count of items inside one section, not a count of documents. Fifty-seven items came close enough to be pulled into an adjudication packet by a deliberately over-inclusive filter, and every one of the fifty-seven was read and coded false rather than filtered out mechanically. Forty-four were read three times, once by each of the two coders working blind from the same packet and once more at adjudication. Thirteen were read only at adjudication, and seven of those come from postings the blind coders never saw. That is the thinnest part of the zero, and it sits at thirteen items out of eight hundred and thirteen.
        </P>
        <P>
        ...
        <P>
          The count is not clean, and where it is dirty changes what you can infer from it. About eight percent of the items resist categorising, and a handful are not requirements at all. One school district&rsquo;s posting runs its qualifications list straight into a payroll term with no heading between them, so the extractor kept reading and counted it. A recruiting mailbox in the same posting is my own fault rather than the employer&rsquo;s: my heading rule matches on the word requirements, it caught a later Job Requirements line, and it swallowed the contact block underneath. The split between the required block and the preferred block is worse than that: on two postings the extraction ignored the employer&rsquo;s own preferred heading, so I would not quote a required-versus-preferred ratio from this pass. None of that moves the zero, because a payroll code cannot be a decision and a misfiled bullet is still a counted bullet. It does mean the eight hundred and thirteen is a working count rather than a clean one, which is the same disclosure the meaning number carries a paragraph earlier.
        </P>
        <P>
        ...
        <P>
          Sixty-four is the denominator because two of the sixty-six have no requirements section to count items in, and the second of those two is awkward for me. One is the posting my own segmenter truncated. The other wrote no requirements section at all, and it is the same posting the next section holds up as the one employer here who states decision rights carefully. So the document that argues hardest against me is the one missing from the denominator of my headline number. The exclusion is real and the reason for it was written down before the count was run. A reader is still entitled to both facts in the same place.
        </P>

        <H2>What happens when the decision does exist</H2>
        <P>
          An absence is a weak finding on its own, and this one has an obvious answer that I want to concede before going further. The answer is that requirements describe the person and responsibilities describe the work, so naturally the decision sits in the other section. That is right. Filing is not a fault, and I am not claiming the decision belongs in one section rather than another.
        </P>
        <P>
        ...
        <P>
          One posting inverts the whole pattern, and it is worth reading closely because it cuts against the easy version of this argument. A quantum computing company&rsquo;s requisition carries a section headed <I>What You Won&rsquo;t Own (by design)</I>, an operating-model section, and a sentence naming what the role holds and what functional owners retain. It is careful, specific writing of the kind this essay is asking for. It is also the single posting in sixty-five that names no attribute required of the person. That was checked against the complete twelve-thousand-character source: there is no qualifications block under any heading, and the only requirements it mentions are export-control ones. So it is the mirror image of the other sixty-four. They say what the person must bring and not what the role decides; this one says what the role decides and not what the person must bring. Of the two questions, every employer here answered one. This employer answered the other.
        </P>
        ...
        <P>
          The first pass ran while the corpus was still being read. Its markers were built out of what was in front of me, so the rule for a decision the hire may make was narrow. It looked for one particular way of handing a role the right to settle something, and it found four postings in sixty-six. That number sat on disk for a week. It is now retired.
        </P>
        <P>
        ...
        <P>
          One piece of that sampling objection can be checked rather than conceded. If these employers were simply too early to have needed decision rights written down, the silence would say something about their stage and little else. So each posting was coded for whether the role holds an AI or machine-learning system in its remit. Twenty-seven of the sixty-five do. Within those twenty-seven, thirteen name a decision the hire may make and fourteen do not. Among the thirty-eight holding no such system, ten name one. That is forty-eight percent against twenty-six, an odds ratio of 2.6, and a two-tailed exact test on the table returns p = 0.11. Sixty-five postings cannot establish a gap that size, so this is a direction rather than a result, and it would be dishonest to report it as more. It cuts both ways regardless. More than half the roles built around an AI system still never say what the person may decide.
        </P>

        <Figure
          src="/library/figures/decision-named-by-ai-remit.svg"
          alt="Two rows of squares, one per posting. Twenty-seven postings hand the role an AI system and thirteen of them name a decision. Thirty-eight hand it no such system and ten name a decision."
          caption="The maturity objection, tested inside the corpus. Holding an AI system goes with a higher rate of naming a decision, 48 percent against 26, at an odds ratio of 2.6. The exact-test p is 0.11, so the table shows a direction this sample cannot establish."
        />

        <P>
          A fourth limit belongs with those three, and it is the one I would go after first if this were someone else&rsquo;s essay. Decision-absence is standing in for something broader. The corpus measured whether a document names a decision the role may make. The argument wants to know whether the document says what work its requirements are for. Those are not the same question. The study that would close the gap has not been run: for each of the eight hundred and thirteen items, is there employer prose establishing what work makes that attribute necessary. It would need its own coding rule, its own double-coding and its own adjudication, so it is a second study rather than a repair to this one. Until it exists, the forty-two counts documents that name no decision, and the step from there to a job unstated is the assumption stated earlier and not a finding.
        </P>
        <P>
          A second arm is designed against that objection and has not been collected yet. It is fifty-six postings, drawn mechanically from an enumerated set rather than picked by me, with the coding rules frozen as they stand. Fifty-six postings can show a large move and cannot resolve a small one. Against a thirty-five percent base rate the arm cannot tell apart anything between roughly twenty-three and forty-eight percent, so it is a check for a big shift rather than a measurement of how far the two arms differ. The falsifier is declared in advance, it sits on the same anchor the argument does, and it is written down where I cannot move it later without the move showing.
        </P>
        ...
        <P>
          That part of the argument is not mine, and it is better admitted than defended. Microsoft put the execution and decision split in the standfirst of its <a href="https://www.microsoft.com/en-us/worklab/work-trend-index/agents-human-agency-and-the-opportunity-for-every-organization" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">2026 Work Trend Index</a>. Deloitte built a chapter of its <a href="https://www.deloitte.com/us/en/insights/topics/talent/human-capital-trends.html" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">2026 Global Human Capital Trends</a> on who is accountable when humans and machines are both deciding. Both are vendor and consultancy research with commercial interests in their own conclusions, and both are worth reading anyway.
        </P>
        <P>
        ...
        <P>
          One requisition carries both of this section&rsquo;s machines at once. It advertises a role that will run AI systems for the employer, and it also describes the AI that screens the person reading it. That posting is the only place in the corpus where a boundary between human and machine judgment is written down cleanly.
        </P>
        <P>
        ...
        <P>
          A fourth feature looked like evidence and is not. The posting also names ten business days, which is the same figure the statute uses. But the statute&rsquo;s ten days is notice owed to the candidate before the tool runs, and the posting&rsquo;s ten days is the applicant&rsquo;s own deadline to write in and opt out after applying. The clock runs in opposite directions. I had it in the list of matches before I checked which way it pointed, and a fourth match would have made the inference look stronger than it is.
        </P>
        <P>
          Now look at what the law asks for, because it lands in a particular place. The notice must tell the candidate which job qualifications the tool will assess. The audit must calculate selection rates and impact ratios by sex, race and ethnicity, and the city&rsquo;s Department of Consumer and Worker Protection, which enforces the law, says in its own guidance that nothing requires an employer to act on what the audit finds. And the tool the statute has in mind, by its own definition, is one that predicts how well a candidate fits the job.
        </P>
        <P>
        ...
        <P>
          Job-relatedness has sat in federal employment law, which is why a municipal audit statute never needed to carry it. That settlement is moving: in June 2026 the Justice Department <a href="https://www.justice.gov/olc/opinion/constitutionality-disparate-impact-liability-under-title-vii" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">advised</a> the Equal Employment Opportunity Commission that its disparate-impact guidelines are unconstitutional. The detail sits outside this essay&rsquo;s question. Neither regime asks what decisions the role holds.
        </P>
        <P>
          That is a claim about where Local Law 144 reaches. It is not a claim that the law is badly built, since it was written to catch disparate impact and it does that.
        </P>
        <P>
          Stand back from the corpus and the shape of it is this. Sixty-six documents advertise roles that will run AI systems inside an organization, and none of them says what those systems may decide without a person. The one clean statement of where machine judgment stops sits in a legal notice, it was written because a regulator required it, and it governs the AI that screens the applicant rather than any AI the job is about. The boundary is drawn at the door and nowhere past it.
        </P>
        ...
        <P>
          <B>To whoever writes or approves the requisition.</B> A requirements section states what a person must bring. Add one line stating what the role settles without another signature, and put it next to the requirements rather than in a section further down. Then what the document asks of the candidate and what it grants the role sit on the same page. That line is what a screening tool has no way to infer, and what a new hire currently reconstructs by trying something and seeing whether anyone objects.
        </P>
        <P>
          One employer in these sixty-six already writes that line, and it is the quantum computing company from earlier. Its operating-model section says the role &ldquo;holds decision rights over IT-owned data architecture, data-platform roadmap, integration standards, cataloging, metadata, lineage, and data-governance workflows; functional owners retain decision rights over domain definitions.&rdquo; Name the decisions, then name what somebody else keeps. Set that next to <I>owns data strategy</I> and the difference needs no explaining.
        </P>
        <P>
...
/>
  ...
  alt="An upper lane labeled the door runs through three boxes: years, tools and titles, then screen, then hire. A lower lane labeled the seat holds one box reading the question this seat answers first. The dashed connector between the two lanes is broken, and the gap is marked unstated."
  ...
  // ===================================================================
  {
    ...
    body: (
      <>
        <Brief>
          <p>
            Organizations keep adding measurement and dropping the part that turns it into a claim. The fix is one accountable charter for the loop. That is the charter that keeps getting split.
          </p>
          <p>
        ...
        <P>
          Measuring more has a bad reputation right now, and it has earned it. Organizations adopt another survey, another dashboard, another data platform, and the claims they can make at the end do not get stronger. Often they get weaker, because there is more to defend and nobody designed any of it against what would need defending. What got dropped was the reasoning part: how to design a measurement, and how to analyze it so that it serves the people running the program, the funder reading the report, and the measurement itself next round.
        </P>
        <P>
          In one organization, research and evaluation designed the theory of change and the instruments, and program teams collected the data. My team, a senior data analyst and a senior data scientist, did the reporting. While I was there, program teams waited months for data they needed the next week to review with clients, and the learning loop ran on anecdotes and meeting notes. How much data was missing came to light only when someone ran an impact analysis. My role was written to hold the seams: near-term program reporting and long-term impact reporting on one infrastructure, and learning loops that would carry collection problems back to research on a regular cadence instead of once, at analysis time. The role was eliminated before that loop could close. <InternalLink slug="every-metric-is-a-claim">Every metric is a claim</InternalLink> follows what happened to the function after that.
        </P>
        <P>
          In a school network, the role sat a level down under instructional technology and became system access. Job 2 survived as dashboards; job 1 went, because what got collected was what the systems already captured, and job 4 went with it. In a behavioral-health agency, the role sat under compliance, with grant reporting as the main deliverable. Job 3 survived in its narrowest form, the report the funder required, and the other three went, because the funder had already chosen the measures. In a fourth organization, the role was eliminated and the work folded into a platform build, which is <InternalLink slug="the-absorbed-data-role">the absorption pattern</InternalLink> with engineering as the absorber. The dashboards got built. Nobody designed against a claim, nobody owed a partner a claim they could check, and nobody revised anything.
        </P>
        <P>
          Across four placements, the loop never had an owner long enough to close. I argued in <InternalLink slug="where-should-data-sit">Where should data sit?</InternalLink> that where a data function reports decides what it is allowed to become, and that it should report to the integration seat rather than to any single function. The principle carries over to measurement with more at stake, because a measurement function owes a claim as well as a number. The four seats above are four single functions. Each kept the job it understood and let the others go, because placement decides which of the four jobs a function can see. A seat inside one function cannot see the loop it belongs to; a seat above the functions can.
        </P>

        <H2>Four jobs across the measurement cycle</H2>
        <P>
          The four jobs form a loop. Job 1 feeds two uses side by side, and job 4 feeds job 1.
        </P>

        <NumList>
          <NumItem n={1}>
            <B>Design the measurement.</B> Decide what to collect before collecting it, against the decisions and claims the evidence will have to support, limit or rule out. This is where the construct gets named and the instrument gets chosen or built. The design exists so the organization can know, at the end, what it may act on and what it may say.
          </NumItem>
          <NumItem n={2}>
            <B>Analyze so it serves operations.</B> The people running the program can act on the result this month. A no-show pattern a clinician can read; a cohort a state team can reach this week.
          </NumItem>
          <NumItem n={3}>
            <B>Analyze so it serves the external party.</B> A funder, a partner, a board gets a claim they can check, with the evidence stated and the strength matched to it.
          </NumItem>
          <NumItem n={4}>
            <B>Revise the measurement.</B> The next round measures better because of what this round showed. Sometimes that is an item that stopped discriminating. More often it is a definition that drifted across sites, a collection protocol, a threshold, or a metric dropped. Sometimes the instrument is fixed by a funder or a statute and cannot be changed. The revision then moves to what surrounds it: who gets measured, what the number is compared against, how it is collected, and what claim it is allowed to carry.
          </NumItem>
        </NumList>
        ...
        <P>
          An objection arrives here from anyone who has built an independent M&amp;E function, and it is a fair one. Measurement is supposed to be split from the programs it measures. Put the people who count outcomes inside the team whose outcomes they count, and the numbers drift toward what the team needs them to be. So is this note arguing against independence?
        </P>
        <P>
          No, because there are two different axes you can cut on. One cut splits the four jobs from each other: design into research, dashboards into operations, the funder report into development, revision into nobody. The other cut splits the function from the programs it measures: all four jobs stay in one charter, and that charter reports outside the teams whose numbers it audits. The first cut breaks the loop, and the second protects it.
        </P>
        <P>
          One charter does not have to mean one team. Research can own the instrument and operations the dashboard. What cannot be distributed is the question of what the data licenses the organization to say. One accountable owner has to hold it and be able to send the answer back to job 1. Job 4 goes missing because nobody in the room is asking for it. Jobs 2 and 3 have someone who will, the program team this month and the funder at the deadline. Job 4 is owed to next year&rsquo;s measurement, which sends no one to the meeting. A council can govern the work, the way the council in <InternalLink slug="the-contracts-between-systems">The contracts between systems</InternalLink> governs who may change a definition; it cannot stand in for the person accountable for the answer, and a committee is often how that accountability disappears. In none of the four seats above did anyone hold it long enough for the answer to come back.
        </P>
        <P>
          The strongest version of the objection is a two-way split: jobs 1 and 4 in one seat, jobs 2 and 3 in another, both under one charter. It is a reasonable proposal, because jobs 2 and 3 already travel together, and that pairing is what a reporting function is. I would expect it to hold on the way out and fail on the way back. Job 4 runs on what jobs 2 and 3 learned in use. That means the field that misled, the caveat the funder queried, the definition that drifted between sites, the dashboard the site team stopped opening. Little of that gets written down, so little of it survives a handoff between seats. A charter that makes the split anyway has to turn the return into an artifact those seats owe to job 4, with someone accountable for its arriving. The closest I have come is the first seat above: the jobs sat with different teams and the role was written to own the return, and it was eliminated before the return had run. So this is a reading of the mechanism, not a finding.
        </P>
        <P>
          One posting makes the distinction cleanly, and it is worth quoting because whoever wrote it was trying to fill a seat. A head-of-data-and-M&amp;E role at an Indian education nonprofit, as captured in May 2026, lists four things under one job. Design the data architecture &ldquo;from field input to impact indicators &mdash; definitions, collection protocols, validation logic, and storage.&rdquo; Build dashboards and report packs for the CEO, state teams and donors. Produce impact reports and evidence briefs. Ensure &ldquo;field learning feeds into policy redesign, process improvement and product updates.&rdquo; That is jobs 1, 2 and 3 under one head, and the learning that job 4 runs on. What the posting does not spell out is the last step, revising the measurement itself; its fourth item feeds learning into the program, not back into the indicators. The role reports to the founder and chief executive, which is the integration seat. The posting says three times that the function must be independent of the program teams it measures, and lists &ldquo;sit under state programs whose performance it must assess&rdquo; under what the role does not do. Its six-month success criteria include &ldquo;at least two key program design changes in direct response to M&amp;E evidence,&rdquo; which is the learning half of the loop written as a hiring metric.
        </P>
        <P>
          That is one document, and it stops one step short of the full loop. I have not found a second that separates the axes this cleanly. The distinction holds on its own logic. What the posting shows is that at least one organization wrote most of it down.
        </P>

        <H2>Job 2 already has a name</H2>
        <P>
          The for-profit world industrialised the middle of this loop and called it business intelligence: operational data turned into something a manager can act on this week, at the level they manage. Job 2 is that discipline in a mission-driven organization. Businesses fight over constructs too (churn, engagement and quality are all contested). The difference is the distance between what an impact organization can observe and the social outcome it has to make claims about. Add the evidence regimes, funder and statutory, that those claims have to survive. BI industrialised much of the operational middle. It did not solve the measurement-and-claim loop around it. In <InternalLink slug="the-contracts-between-systems">The contracts between systems</InternalLink> I described the leader looking for institutional intelligence (a 360 view for the CEO, a number a district officer can act on this week). The integration ran, and the institution still could not make the decision. The gap there was governance: who reads the number, who can change its definition, who has to be told. This note takes up the other half: whether the number was ever built to be acted on.
        </P>
        ...
        <P>
          A warehouse, a semantic layer and a dashboard tool make job 2&rsquo;s output cheap. A reporting pipeline with a model drafting the narrative does the same for job 3. All of that makes it easier to skip matching the strength of a claim to its evidence. Bridgespan&rsquo;s practitioner guide to nonprofit MEL, written from work in Africa, Asia and the United States, grants that technology can cut the burden of collection. The same sentence calls collecting more and more data &ldquo;a temptation generally to be avoided&rdquo; (<a href="https://www.bridgespan.org/insights/nonprofit-organizational-effectiveness/a-practical-guide-to-nonprofit-measurement-evaluation-and-learning" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">Searle et al., The Bridgespan Group, 2025</a>). Its test for a proposed metric puts a decision question beside the burden question: what decision can&rsquo;t we make because we are missing this information today? That is job 1 as a screen, written before the drafting models made the middle cheaper still.
        </P>
        <P>
          Now the organization adopts more measurement because collecting and displaying it costs almost nothing, and the claims do not get stronger, because nothing in the stack owns the loop. A model can propose what to collect, flag an item that stopped discriminating, and draft a replacement. What no tool holds is the decision that follows. Does the revised measure still represent the construct? Did the change break the trend line? Does the evidence now license the claim the funder is about to read? Those decisions sit at jobs 1, 3 and 4, and none of them belongs to the stack. They need an owner, and the cheaper production gets, the easier it is to run an organization that never names one.
        </P>
        <P>
          The vehicles do their job. They carry jobs 2 and 3 faster than any team could. They also make a broken loop look like a working one, because the dashboards refresh nightly and the reports go out on time. <InternalLink slug="the-absorbed-data-role">The absorbed data role</InternalLink> is about who ends up holding the plumbing when nobody is hired for it. The four jobs sit one step further along. Even a funded plumbing role, the curation and governance seat that note argues for, does not hold job 1 or job 4. Curation and governance keep the data sound; they do not decide what to collect or change it next round.
        </P>
        ...
        <P>
          <B>If you fund or govern an organization</B>, ask four questions of its measurement, in this order. Who decided what to collect, and against what question? Who can act on the result this month? What claim can we check, and on what evidence? What changed in the measurement since last round, and who changed it? An organization that answers the first from research, the second from operations, the third from development, and the fourth with silence has split the loop. Its dashboards may still be very good.
        </P>
        <P>
        ...
        <P>
          I have watched the charter that holds all four get written, and then split four different ways. Each split was reasonable on its own terms. The loop was the thing nobody wrote down.
        </P>
        ...
        <SeeAlso>
          ...
          <SeeAlsoItem
            ...
            gloss="The time half of the same problem: what a metric is licensed to say between evaluations, and who decides when that license is rechecked."
          />
        </SeeAlso>

        <MetaNote>
          Written September 2026 for the Analytic Bytes Library. The four placements are from the author&rsquo;s own record; sectors are named and employers are not. The job posting is quoted as captured in May 2026 and the organization is not named. The Randari, Trivedi, Yurkofsky and Bridgespan pieces were checked against the published pages, with snapshots held in the piece&rsquo;s source pack. The Messick and Kane citations are carried over from the AB measurement arc, where they were verified.
        </MetaNote>
      </>
    ),
  },
  ...
  // ===================================================================
  {
    ...
    body: (
      <>
        ...
        <P>
          In August, I read <InternalLink slug="the-floor-is-the-frontier">Machines of Loving Grace</InternalLink> as a specification with the acceptance criteria left out. It said what powerful AI could do for people. It did not say how anyone would know it had: what to measure, for whom, by when, and checked by whom. In September, Dario Amodei published something closer to that. <a href="https://darioamodei.com/post/we-must-pace-the-frontier" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">We Must Pace the Frontier</a> argues that AI labs should slow down the rate at which model capabilities improve, so that safety work can keep up, and it describes how anyone would check that safety work had kept up. He has not published the finished test, but he has published its architecture: a trigger, an evidence requirement, an independent verifier with access to the production process, and the standing to say in public what the verifier found.
        </P>
        <P>
          A specification for powerful AI has two halves. It should not hurt anyone, and it should help people. The new essay writes the check for the first half. The people the second half is about, the floor in the earlier note&rsquo;s terms, still have no check at all.
        </P>
        ...
        <P>
          This is the outline of something checkable, which many of the people who worry about AI risk have asked for over several years. My argument is about direction.
        </P>
        ...
        <P>
          Safety is the essay&rsquo;s subject, and a safety essay is allowed to leave benefit alone. What the essay shows, by doing one half so well, is how little the other half has. The risk side has moved from a claim to a gate. The benefit side has a sentence in paragraph one.
        </P>
        <P>
        ...
        <P>
          For the benefit loop, it buys one real thing. A slower frontier means fewer model versions mid-study, so the model tutoring a child in month one is still the model being evaluated in month twenty. An evaluation whose treatment changes halfway through cannot say what it evaluated, so holding the intervention still is a precondition for the benefit test rather than the test itself. The test still needs three things pacing does not touch: a definition of what counts as the floor rising, a way to reach the people the deployment was meant for, and the wait for the outcome to appear. Whether a child reads at grade level two years after a tutoring deployment takes two years to observe, whichever model did the tutoring and however slowly the next one arrives.
        </P>
        <P>
          The objection usually arrives as continuous evaluation: embed the evaluators throughout deployment and validate in real time, without waiting years. Embedded evaluators can watch the process continuously (the model, the prompts, the retrieval sources, the risk flags), and that is worth having. They cannot shorten how long the outcome takes to appear. So there are two clocks. A process clock runs all the time, and an outcome clock still waits. The claim <I>this helped</I> belongs to the second.
        </P>
        <P>
        ...
        <P>
          The detail in the essay that matters most for the benefit side is the desk. Embedded evaluators get a desk, a badge, a laptop, and roughly the same permissions as the internal team. That is an unusual thing for a company to offer, and he says so. It is also a precise description of what evaluating an outcome claim requires. Access to the pipeline behind the number. The standing to ask the people who ran it what they did. The right to say afterward what was found.
        </P>
        <P>
          I have sat in the access half of that seat, on the benefit side. I ran data for a K-8 charter network expanding into districts with sixteen to seventeen percent English learners, up from the eight to ten where we had been. We set an enrollment target closer to twelve, on the basis that our experience should carry. We enrolled seven or eight. Seven against twelve looks like a shortfall, but twelve was an aspiration, not a commitment. Nobody had said, and I include myself, what serving those students would mean once they were enrolled, or what share made that worth testing, or what would follow if we came in under. Without those, seven was not a miss. It was the number. I had the desk and I could see the pipeline, and the only thing the seat could do with what it saw was report it. An AI deployment would sit in the same seat, with the model version added to the things that can shift underneath.
        </P>
        <P>
          That is the state of the benefit side now. The harm side is getting the seat by commitment, in writing, at two labs, as a condition of proceeding. The benefit side has seats: the evaluation a funder requires, the assessment and accountability lead a school network hires, the compliance officer a clinic keeps for the auditor. The case for independent evaluation of AI in social programs has been made, and it asks for evaluation to run alongside deployment (<a href="https://ssir.org/articles/entry/ai-for-good-impact-evaluation" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">Dhaliwal and Hou, Stanford Social Innovation Review, 2025</a>). What it does not ask for is the evaluation as a condition of the claim. Each of those seats exists because someone chose to pay for it. A funder can require an evaluation or not; a network can hire the assessment lead or not; a clinic can keep the compliance officer or not; and a program can say <I>this helped</I> either way. On the risk side, under the pacing plan, a model does not move past capability X until an evaluator has checked, and the check is the price of moving. The benefit side has the seat as an option. It does not have the seat as a condition of making the claim.
        </P>
        <P>
        ...
        <P>
          The objection is that these programs, including ones I have worked inside, already evaluate. Many do, and some of it is careful. But look at who the evidence is about. A charter network&rsquo;s outcomes are measured on the students who won the lottery and enrolled; the claim is made for the district. A campus mental-health program&rsquo;s results are measured on the students who answered the survey, and answered it straight; the claim is made for the youth who need it, who are the least likely to do either. That is real evidence for a claim about the people inside the filter. The students inside the filter may well be better off, and that is worth saying, at its size. It becomes evidence for the people the claim names only by an inference, and the gate asks for that inference to be written down before the claim is made, not found afterward.
        </P>
        <P>
    ...
    body: (
      <>
        <Brief>
          ...
          <p>
            This essay makes that case from one organization&rsquo;s requisitions before and after the seat was cut, eight postings already hiring for it, and Kane&rsquo;s argument-based approach to validity. It ends with three fields to write on any metric that is being read as if it said something about an outcome.
          </p>
        </Brief>
        ...
        <P>
          The organization kept its evaluation function and its reporting function. What it could not do, in the time I was there, was act this quarter on a number it could also defend. I cannot speak for what happened after I left, and the only evidence I have is the postings, which divide the work further than it was divided before. That is the gap this essay is about. It is not the ownership gap that <InternalLink slug="four-jobs-one-loop">Four jobs, one loop</InternalLink> takes up. It is a gap in time. The evidence that would answer the question exists at two moments, and neither moment is the one the decision is made in.
        </P>
        ...
        <P>
          The evaluation field knows this. J-PAL&rsquo;s <a href="https://www.povertyactionlab.org/ai-evidence-playbook" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">playbook for AI-enabled programs</a> says that AI systems generate rich engagement, monitoring and technical-performance data, &ldquo;but these signals alone cannot show whether an intervention improves outcomes in the real world&rdquo; (executive summary, last updated 17 February 2026). That sentence states the problem from one side. The field split monitoring from impact evaluation on purpose, and the split is reasonable. What happens next is organizational rather than intellectual. In practice the evaluation gets a design, a counterfactual, a pre-registered outcome and a warrant for its claim. Monitoring gets governed as a data-quality problem (accuracy, completeness, timeliness). Those are properties of the data. Validity is a property of the inference, and in the organizations I have worked in it became explicit only on the evaluation side, never on the half of the system that runs every day.
        </P>
        <P>
          The funder side has noticed the gap in time from its own end. Lucy Brainard, <a href="https://ssir.org/articles/entry/capacity-building-evidence-standard" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">writing in *Stanford Social Innovation Review</a> about how foundations evaluate the capacity building they pay for, proposes three horizons: satisfaction at the end of the engagement, behavior change at six to twelve months, and performance at a year or more. The middle horizon, she says, is the one funders routinely skip, and it is the one where the learning is. That middle horizon is the number between evaluations, seen from the grant side.
        </P>

        <H2>One argument, two halves</H2>
        <P>
          Kane&rsquo;s argument-based approach (<a href="https://doi.org/10.1037/0033-2909.112.3.527" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">1992</a>) gives the general principle. Validity attaches to an interpretation and a use rather than to an instrument, and it is established by a chain of inferences: from the observation to a score, from the score to the wider domain, from the domain to the construct, from the construct to the decision. Each link needs a warrant. <InternalLink slug="the-valid-dollar">The valid dollar</InternalLink> read that chain at its weakest link for one fund&rsquo;s evaluation spend, and <I>The construct keeps moving</I> followed what happens to the chain when the construct itself shifts under a measurement architecture that does not.
        </P>
        <P>
          Translate the chain onto a data stack and the pipeline steps can be read through the same logic. The collection protocol and the validation rule have the structure of a scoring claim. The dashboard rollup by site or cohort has the structure of a generalization claim; it asserts that this aggregate speaks for those people. The semantic-layer definition, where an aggregate of observed fields is read as &ldquo;housing stability,&rdquo; has the structure of an extrapolation, from what was counted to the construct the organization cares about. The funder decision the dashboard drives is the use. Every one of those steps changes what the number means, and in most stacks I have seen none of them is treated as a claim. I am borrowing Kane&rsquo;s scaffolding. His inferences do not describe a data pipeline one for one, and they do not by themselves say how an early signal is tied to a later outcome. That empirical problem has its own traditions, and they split along the same line as the two claims below. Prognostic modeling and leading-indicator work address whether an early signal predicts a later outcome well enough to act on. Surrogate-endpoint validation addresses the much stronger question of whether movement in an intermediate measure can stand in for movement in the final one. Kane says a claim needs a warrant, and those traditions say how one is built. None of them puts the warrant where the organization reads the number.
        </P>
        ...
        <P>
          Organizations are already hiring for the join, even where no charter describes it. Eight postings since May, seven of them captured this week, describe the seat. Four are worth reading closely.
        </P>
        <P>
          A pan-African education and workforce group is hiring a head of impact to lead &ldquo;the evolution of our Impact function from a reporting and monitoring function into a world-class evidence, learning, validation, and impact intelligence capability.&rdquo; The posting wants randomized and quasi-experimental methods, tracer studies and longitudinal research. It wants the same person to strengthen &ldquo;data pipelines and verification systems&rdquo; across surveys, platform data, the CRM and employer feedback. It asks for an impact team that is &ldquo;proactive, responsive, and embedded across the organization rather than functioning as a retrospective reporting unit.&rdquo; And it writes the dated-warrant idea straight into a dashboard requirement: data products that can &ldquo;distinguish between actuals, estimates, projections, and externally validated outcomes&rdquo;. That is four grades of warrant on one screen, written by a hiring manager.
        </P>
        <P>
          A New York social-services nonprofit with a staff of nine hundred is hiring a vice president of program evaluation and quality assurance. The seat sets &ldquo;the vision of business intelligence and performance analytics,&rdquo; shares responsibility for &ldquo;the organization&rsquo;s data ecosystem and analytics architecture&rdquo; with the chief information officer, and runs quality assurance, program evaluation and incident management. The seat reports to the vice president of IT. The placement, under IT, is the question <InternalLink slug="where-should-data-sit">Where should data sit?</InternalLink> was written about. And the incident half is fully built out in the text (reporting tools, workflows, dashboards, regulatory standards, trend analysis) while the outcome half is &ldquo;evaluation findings&rdquo; to be translated. The risk claim has continuous machinery; the benefit claim has a report. That is the asymmetry <InternalLink slug="the-gate-runs-one-way">The gate runs one way</InternalLink> describes at the AI frontier, sitting inside one department of a homeless-services agency.
        </P>
        <P>
          A funder this library has already written about is hiring for the calibration itself. The co-author of the impact metric <InternalLink slug="the-valid-dollar">The valid dollar</InternalLink> responded to wants an impact modeling analyst to build the ROI models behind its target of more than $100 in lifetime earnings per dollar spent, with &ldquo;appropriate counterfactual assumptions,&rdquo; and in the same seat to manage &ldquo;the Foundation&rsquo;s core datasets of grantee impact data&rdquo;. One duty appears in no other posting in the set: &ldquo;Compare modeled expectations with realized investment outcomes and use findings to strengthen future models, assumptions, and investment decisions.&rdquo; That is the recheck, written as a job. What the posting does not say is when. Assumptions are refined &ldquo;based on investment outcomes, new data, research findings, and organizational learning,&rdquo; a list of triggers with no schedule and no threshold.
        </P>
        <P>
          The fourth comes closest to the method. A senior program officer for data and AI infrastructure at a large foundation is to identify and broker the dependencies between two portfolios, among them &ldquo;the need for evaluation approaches to be designed in concert with the data systems that will collect outcomes&rdquo;. That clause is the join, named. If the study is to establish the link the dashboard will carry, the study has to be designed with the dashboard&rsquo;s signal in it, whether that signal was chosen at the outset or found in the data later. The posting names that as a coordination problem between two portfolios, to be brokered. No charter carries it as a method.
        </P>
        <P>
          The other four postings put evaluation, dashboards, governance and data infrastructure into one seat and disagree about where it reports. A New York anti-poverty organization places it under the chief program officer. A large family foundation places it under the head of finance and operations. An Indian education nonprofit places it under the founder and chief executive, and asks the same seat for real-time dashboards and periodic evaluation cycles. And a second foundation&rsquo;s principal officer for measurement, inside the president&rsquo;s office, is to maintain a &ldquo;ladder of outcomes&rdquo; with leading indicators on it, without saying what licenses an indicator to stand on the ladder. Add the social-services nonprofit above, under IT, and that is four reporting lines for one seat. <InternalLink slug="where-should-data-sit">Where should data sit?</InternalLink> argued that where a function reports decides what it is allowed to become, and this one has not been decided.
        </P>
        <P>
          Eight postings I went looking for cannot show a trend. What they show is the same shape appearing across eight organizations and three reporting lines, before its operating method has a name. I have not found that method in the field. The evaluation literature has the warrant and no infrastructure practice. Semantic-layer practice is good at recording what a metric means and rarely records what claim the metric is licensed to support. Whoever takes these seats will be writing the join themselves. The organization that eliminated the seat I held moved the role under research, where the evaluation half is comfortable and the infrastructure half is somebody else&rsquo;s. It is the pattern <InternalLink slug="who-writes-the-contract">Who writes the contract?</InternalLink> describes: an impact-anchored placement that holds while its champion is there and dissolves back into the silos when the champion leaves. The twenty-three statements did not go to one seat. The evaluation half went to two seats under research, and the infrastructure half went to technology. The seat I held had sat on the infrastructure and driven the semantics on it, so that the program team could stop mining its own numbers out of instruments and spreadsheets, and research could read the same numbers for impact reporting and the learning loop. Now technology runs the stack, research keeps its own definitions for the impact reporting, and nobody holds the layer that says what a number is licensed to mean. The postings above move the other way. Both directions exist in the same year, and neither has the method.
        </P>

        <Figure
          src="/library/figures/every-metric-is-a-claim-two-halves-one-program.svg"
          alt="Two halves, one program"
          caption="The program sits between the half that can warrant a number and the half that can carry it. The warrant is the one thing that has to cross, and no seat is chartered to carry it."
        />
        <H2>Why the constrained organization needs it most</H2>
        <P>
          The objection is that this is a rich organization&rsquo;s problem. A foundation with a data science team can afford a ladder of outcomes and a recheck schedule. A homeless-services agency with nine hundred staff and a $140 million budget cannot. It should keep its evaluation function, keep its reporting function, and get on with the work.
        </P>
        <P>
          I think the opposite, and the reason is time. Every participant passes through once, whoever funds the program. What money buys is slack: a next cohort, a budget to redesign for it, room to absorb the wait for a fourteen-month outcome. Where slack is scarce, the cost of waiting is harder to absorb, and the organization acts on the month-two number anyway, with or without a warrant. The person in the shelter bed, the young adult six months out of foster care, is served now or not served, and by the time the outcome study reports, that person is gone. A dashboard that says attendance is falling in month two, carrying an operational warrant from a study that tested it, is the difference between an intervention in month three and a finding in month fourteen. The organizations with the least room are the ones for whom a warranted early signal is worth the most. They are also the ones least likely to have anyone chartered to build it, which is why they end up where <InternalLink slug="the-reach-trap">The reach trap</InternalLink> left them: back-breaking collection, reach to show for it, and no claim about what moved.
        </P>
        <P>
        ...
        <P>
          Data infrastructure and AI make the everyday work cheap: collecting the data, building the dashboard, drafting the report. <InternalLink slug="four-jobs-one-loop">Four jobs, one loop</InternalLink> already argues that cheaper production does not supply an owner for the reasoning around it. The narrower question here is how often the link can be re-established.
        </P>
        <P>
        ...
        <NumList>
          <NumItem n={1}>
            <B>The claim it is licensed to support.</B> Stated as a claim, in the organization&rsquo;s own words, and graded. &ldquo;Month-two attendance is validated as an early-risk indicator for month-fourteen employment&rdquo; is one claim. &ldquo;Rising attendance is evidence that employment is rising&rdquo; is a different and stronger one. If nobody can fill this field, the metric is a count, and the dashboard should say so.
          </NumItem>
          <NumItem n={2}>
            <B>The evidence behind it, and the date.</B> The study or analysis that established the relationship, its design, the population it was established in, the strength of the relationship, and when. Two states of this field look alike on a dashboard and are not: found in historical data, and held up out of sample on a later cohort. Say which. A citation, an internal analysis with a date, or the word <I>none</I>. The pan-African posting&rsquo;s four grades (actuals, estimates, projections, externally validated) are a coarser version of this field, and a good one.
          </NumItem>
          <NumItem n={3}>
            <B>When it must be rechecked, and who decides.</B> The condition or date that puts a recheck due: a program redesign, a change in eligibility, a population shift above some threshold, the signal becoming a target, or a fixed interval. And the name of the person who owns the decision when it fires. Operationally this is an expiry, and what expires is the permission to rely on the evidence unchecked. Without the owner, the trigger fires into the same silence <InternalLink slug="four-jobs-one-loop">Four jobs, one loop</InternalLink> describes; with it, the field forces the decision that neither the evaluation team nor the data team is chartered to make today.
          </NumItem>
        </NumList>

        <P>
          Three fields go on the metric, in the layer the dashboards read from. An evaluation report in a shared drive cannot carry them, and a data dictionary describes the column, not the claim. A funder can ask the same three things of any dashboard number a grantee shows them. If a grantee reports month-two attendance at 82 percent and cannot fill field 2, the funder knows what that number is: a count of attendance, accurate and current if the data quality holds, and nothing yet about employment. <InternalLink slug="the-valid-dollar">The valid dollar</InternalLink> has the table for how strong a claim in field 1 the evidence in field 2 can support; this essay is about where the three fields live, and field 3 is what keeps them current.
        </P>

        <H2>Two floors away</H2>
        <P>
          At the national nonprofit, the reporting prototype I built ran a model against the semantic layer and mapped its output to the theory-of-change domains, so that an impact narrative could be drafted from live evidence rather than written after the fact and back-justified with the metrics that happened to move. It worked. What it could not do was tell the reader which of those live numbers had ever been shown to say anything about the outcome in the domain heading above it, or which claim each one was licensed to support. The prototype had the carrying half of the argument and not the warranting half. The evaluation team, two floors away, had the warrant and no way to put it into the layer the prototype read from. The program team between them got no number at all. Nobody could decide whether to act on the month-two signal or wait fourteen months for the outcome, so the program did its own data management, pulling together what it could to give its clients some insight and to shape delivery for the months ahead. No seat was chartered to carry the warrant across. The requisition that replaced the role moved it under research, where the warrant is comfortable, and the semantic layer is technology&rsquo;s.
        </P>
        <P>
          The evidence establishes the warrant. The infrastructure carries it. Somebody decides when it must be checked again. Those three sentences are the whole method. Split them across two seats, as the organization I left did, and the warrant falls between them. Put them in one seat, as the postings do, and that seat still needs the third sentence written into its charter: who decides when the evidence must be checked again. None of the eight says. Until one does, the number on the dashboard is still a claim, and nobody has signed it.
        </P>
        ...
        <MetaNote>
          Written September 2026 for the Analytic Bytes Library. The eight job postings were captured between May and 14 September 2026 and are held as snapshots in the piece&rsquo;s source pack. None of the eight organizations is named. The organization in the opening and the reporting prototype are from the author&rsquo;s own record and are not named either. The J-PAL quotation was checked against the playbook&rsquo;s full text, and the Brainard piece against the published article; the simulation named in the AI section is logged in the AB idea log and has not been built.
        </MetaNote>
      </>
    ),
  },
  ...
  // ===================================================================
  {
    ...
    body: (
      <>
        <Brief>
          <p>
            Two people had different absence numbers for the same month, and neither had made a mistake. One number came from the student information system, which counted what it had fields for. The other came from a worksheet kept by the staff who take attendance, which knew why a child had been out. The figure going to the authorizer had to come from one of them.
          </p>
          <p>
            The systems in these organizations are kept for the record, for a state or a payer or an authorizer. The people doing the work keep their own spreadsheets, because the system was never built to hold what they need in order to act. That split is not a failure of anyone&rsquo;s diligence. It is what the systems were bought for.
          </p>
          <p>
            AI makes it cheap to translate what those spreadsheets know into explicit rules, and to carry one record into the other. It does not supply the person who decides whether the rules are right, or the discipline that reconciles the two numbers. This essay is about what to ask for.
          </p>
        </Brief>
        ...
        <P>
          Network operations and the person who owned the system&rsquo;s configuration had two different absence numbers for the same month, and neither of them had made a mistake.
        </P>
        <P>
          The student information system counted what it had fields for. A day was present or absent. The worksheet network operations kept, fed each week by the school ops staff who take attendance at the sites, carried what the system could not hold: the half-day suspension the system books as a full absence, the tardy that was never converted after somebody checked the sign-in sheet, the unexcused absence that became excused once a staff member called the family and found out what had happened. None of those were corrections to the system&rsquo;s data. They were the things the system had no field to ask about.
        </P>
        <P>
          The number was going into a report the chief operating officer had to sign, for the network&rsquo;s authorizer. The figure network operations sent up did not match the one the system produced, and the person who owned the system&rsquo;s configuration could not say which was right. It came to me, to reconcile the worksheet against the configuration before anybody could sign anything.
        </P>
        <P>
          Nobody in that exchange disagreed about what an absence is. They disagreed about which record held what had actually happened, and the more complete one was kept by a person, in a file the institution did not own.
        </P>
        <P>
          That is not a failure of anyone&rsquo;s diligence. The system was chosen because the state required it, and it was good at producing the report the state wanted. It was not built to record that a suspension was half a day, or that a family had an explanation nobody had heard yet on Monday. The people came first. The system came second, for the record.
        </P>
        <P>
          That ordering is correct, and I would not argue with it. When the mission is a person in front of you, the system that documents the encounter loses to the encounter every time. The consequence is a particular kind of organization, one I have worked inside in K-12, higher education, community behavioral health and a national youth mental-health nonprofit. This essay is about what that ordering does to the systems, and what the people do in response. Then it asks what AI changes, when there is no budget to replace the system and no funded data role to run it.
        </P>
        ...
        <P>
          One group is kept current, even where every entry is manual. These are the systems an obligation can only be discharged inside. A special-education billing claim is filed from within the billing system, against each entry, and nothing outside it files the claim. Enrollment events, a transfer in or a discharge or a withdrawal, are recorded in the district&rsquo;s own student information system, which school staff reach with credentials that district issues. A local file satisfies neither obligation. Somebody has to log in.
        </P>
        <P>
          The other group can be fed later. Attendance is taken on paper or in a sheet and uploaded at the end of the week. A behavior incident is written down somewhere and entered in retrospect. The platform receives a record that was made elsewhere, which is this essay&rsquo;s own subject arriving one level up. The sheet is the working copy and the platform is where it eventually lands.
        </P>
        <P>
        ...
        <P>
          Which turns the phrase over. These organizations are not keeping systems for the record. They are keeping them for somebody else&rsquo;s record. In one network I worked in, the operations lead logged into the district&rsquo;s system to record a transfer or to track a withdrawal, and then entered the same event by hand into the network&rsquo;s own student information system. The authoritative version lived outside. What the organization held was a copy, made by a person, downstream of a system it did not control. And the credentials for the system that mattered sat with two or three people, which is its own exposure the first time one of them leaves. The credential boundary is an analytic boundary too. I never logged into either district system myself. What reached me was a monthly spreadsheet, exported by the team that held the credentials, and the dashboards built on it could never be fresher than that export. The people who could see the authoritative record could not analyze it. The person who could analyze it could not see the record. That is why the reporting layer in these organizations goes quiet, and it is not neglect.
        </P>
        <P>
        ...
        <H2>Systems for the record, spreadsheets for themselves</H2>
        <P>
        ...
        <P>
          They keep their own spreadsheets. Systems for the record, spreadsheets for themselves. Not out of laziness, and not because nobody trained them. The enrollment director keeps a sheet because the system&rsquo;s applicant count does not match what she knows from the call center. The program manager keeps a sheet because the code she needs was added to the system after her staff had been trained on the old one, and half of them still log the old one. The finance lead keeps a sheet because Intacct was never connected to anything else and the numbers in the warehouse are a copy of a copy. Each owner trusts the sheet they built, because they can see every row and they know what each column means. They do not trust the system, because it was built for someone else&rsquo;s report and it changes under them.
        </P>
        <P>
          Higher education has a name for the tool version of this. Novid Parsi, writing about shadow IT, argues that faculty install unsanctioned tools not out of disregard for policy but because the sanctioned path is slower than the need, which makes the unsanctioned tool a reading on the sanctioned one rather than a compliance failure (<a href="https://edtechmagazine.com/higher/article/2026/09/what-shadow-it-can-teach-higher-ed-tech-leaders-about-faculty-experience" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">Parsi, EdTech: Focus on Higher Education</a>, September 2026). He names the risk that follows, too: someone installs a tool, leaves, and a process now depends on something nobody can support. The spreadsheet is that same reading taken on the record instead of on the tool, and the difference matters. A tool someone adopted can be replaced. A record someone keeps privately is the organization&rsquo;s own account of what happened, held where the organization cannot see it.
        </P>
        <P>
        ...
        <P>
          The cost of producing each of those artifacts has dropped. The cost of owning them has not. Two surveys of different populations show the shape of the risk rather than its size. dbt Labs asked analytics engineers what they prioritize and found 72 percent on AI-assisted coding against 24 percent on AI-assisted pipeline management (<a href="https://www.getdbt.com/resources/state-of-analytics-engineering-2026" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">dbt Labs, *State of Analytics Engineering 2026</a>). Among 346 nonprofits surveyed in late 2025, 92 percent use AI, 81 percent use it on an ad hoc basis, and 7 percent report major improvement in their ability to achieve their mission (<a href="https://virtuous.org/blog/2026-nonprofit-ai-adoption-report/" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">Virtuous and Fundraising.AI, 2026 Nonprofit AI Adoption Report</a>). Neither survey measures what I am claiming, and read together they are consistent with it. My reading is one owner at a desk, using a model to get through Tuesday faster, and writing nothing down. The constraints ease for that person. The organization keeps the same box, with one more version of the number in it.
        </P>
        <P>
          Matt Hennessey, chief data and analytics officer for NHS Greater Manchester, states the condition directly in an argument about where decision rights should sit. Local control without local capability, he writes, &ldquo;is simply the transfer of responsibility without the means to discharge it,&rdquo; and the unit of currency should be capability rather than product (<a href="https://www.linkedin.com/pulse/nhs-does-need-one-data-platform-rule-them-all-needs-matt-hennessey-srdee/" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">Hennessey, LinkedIn</a>, August 2026). His subject is a health system rather than a spreadsheet on a desk, but the condition is the same one. Keeping the sheet with the owner is right; calling that the answer is what moves the responsibility without moving the means.
        </P>
        <P>
        ...
        <P>
          The finance and HR wall is that in its hardest form, and the pitch will imply a model dissolves it. A model does not open Intacct. The security narrative is a governance position, and the only thing that answers a governance position is a specific one: these fields, to this warehouse, read by these roles, under this contract, with this audit trail. That is the governance contract <InternalLink slug="the-contracts-between-systems">The contracts between systems</InternalLink> describes, and it is written by people with authority. What a model can do is draft the specific proposal, from the two schemas and the questions the organization wants answered, so the conversation is about a named list of fields rather than about &ldquo;security&rdquo; in general. In my experience the narrative has survived because nobody made it specific. Drafting the specific list is now a model&rsquo;s first pass. Deciding what goes on it is not.
        </P>
        <P>
          I have argued across this library that organizations fail when decision load exceeds decision capacity, and that capacity is an architecture you build rather than a headcount you hire (<InternalLink slug="decision-load-vs-capacity">Decision load vs decision capacity</InternalLink>). So: does AI add capacity inside the box? It does, and the seam is where. But capacity only counts if it lands in the architecture. A model that drafts a crosswalk for one owner who keeps it on her desktop has added capacity to her Tuesday. A crosswalk that is confirmed, versioned, filed where the next person finds it, and run by the system entry every cycle has added capacity to the organization, because the next owner inherits it and the compliance report draws from it.
        </P>
        <P>
        ...
        <P>
          School ops turns over, and network operations trains whoever comes next. The worksheet survives all of it, because a file does. What does not survive is the part that made the worksheet worth more than the system: knowing which rows had been amended, on whose understanding, and why. The columns stay and the reasons leave with the person who entered them.
        </P>
        <P>
          So the next time the report comes round, somebody reconciles the two numbers again, from the beginning, by hand. That is what I was doing that day, and somebody had done it the year before, and nothing either of us worked out was written down anywhere the next person would look.
        </P>
        ...
        <MetaNote>
          External sources were verified on 2026-09-15 against the live pages for Anthropic (Cherry, Peng, Jiao, Leder and Chang, June 3, 2026), Healthcare IT News (Siwicki, August 7, 2026, reporting a Carta Healthcare survey), dbt Labs (State of Analytics Engineering 2026, fielded December 2025 to February 2026), Virtuous and Fundraising.AI (346 nonprofits, December 2025), Databricks (Mherabi, May 13, 2026), CIO.com (Gandhi, June 22, 2026), Snow (August 27, 2026), Ustimov (Decision Spine, August 21, 2026) and Durgin (Think Solve Repeat, September 13, 2026; added after the Standard pass at Chaitanya&rsquo;s pointer, not yet in the knowledge repo). Novid Parsi (EdTech: Focus on Higher Education) was read live on 2026-09-25 for the shadow-IT lineage; author, date and both claims confirmed, and the passage paraphrases rather than quotes because the article&rsquo;s key sentences are spoken by a named interviewee whose attribution was not separately checked. Matt Hennessey&rsquo;s post was read in full on 2026-09-25 in the browser, after the automated fetcher was refused by the site&rsquo;s robots file; author, title, date of 25 August 2026 and both claims were confirmed against the page, and the quoted sentence is verbatim. The repo record kr-2026-08-25-002 had paraphrased it accurately. Because LinkedIn Pulse can be edited without version history, a full text capture is filed at _working/snapshot_hennessey_devodata_2026-09-25.md per the snapshot-durability rule. The MIT Sloan Management Review piece was read in full on 2026-09-16 (Grossman, May 28, 2026); the claim in the text was narrowed to the survey figure the article actually reports, and the survey is the author&rsquo;s own firm&rsquo;s, run with The Harris Poll, which is stated in the text. Scott is cited from the repo entry, which was itself verified against secondary sources; this essay paraphrases chapter 9 and quotes nothing, and chapter-level attribution is accepted for the paraphrase (Chaitanya, 2026-09-16). Two rounds of external adversarial review (GPT, 2026-09-15) shaped v0.4 and v0.5: trusted surface distinguished from authoritative truth, the round trip made central, the model-proposes / human-confirms / code-preserves division stated, the seam versioned, a write boundary added, the second half compressed, the funder ask reversed. The keep-the-spreadsheet-and-carry-it-in move and the systems-box picture are Chaitanya&rsquo;s, from the drafting pours of 2026-09-15. The inside-or-outside discriminator in Where the trust went is also hers, from an operator review of a school-network platform portfolio on 2026-09-18 and from her own seat at a New York City charter network, stated here in de-identified form with no client or engagement specifics. A first version of that passage turned on whether a number was extracted and how often; she corrected it, because attendance and discipline were recorded outside the system and uploaded in retrospect while the district and billing systems required a credentialed login. The corrected discriminator is whether an obligation can be discharged from outside a platform or only from inside it. A scoped Gate 5 run on 2026-09-18 verified the Civil Rights Data Collection against the Department of Education&rsquo;s own page: a mandatory, generally biennial survey of all public school districts, administered by the Office for Civil Rights, covering student discipline alongside enrollment and school climate. The district system is described generically rather than named, and the passage carries no claim about how many states require classroom observations, because that prevalence was not checkable at reasonable cost and the argument does not need it. Seat examples reuse details already published in <I>The contracts between systems</I>, <I>The numbers don&rsquo;t agree because the words don&rsquo;t</I> and <I>Extending the gates</I>. The opening is a reconciliation Chaitanya ran; the authorizer and the funder are described by role rather than named, consistent with every other seat in this essay. Outside-source naming is split deliberately: the list in <I>What the standard advice assumes away</I> names where different kinds of context live and is not a claim about what she pulled, while the list in <I>The outside</I> is narrowed to the state education department&rsquo;s reporting and the National Student Clearinghouse, which are the two she worked with and the only two that bear on an enrollment question.
        </MetaNote>
      </>
    ),
  },
