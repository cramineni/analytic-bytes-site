  ...
  // ===================================================================
  {
    ...
    body: (
      <>
        ...
        <P>
          So the minimum is a familiar move. It is how a field with more at stake handles the same arithmetic problem.
        </P>
        <P>
          The analogy has a limit, and the limit cuts against the rule. GRADE&rsquo;s outcomes are entangled: mortality, hospitalization and adverse events all come from one intervention given to one set of patients, so weak evidence on one constrains what can be said about the balance. Impact pathways are separable. Legal fees avoided for one group does not depend on housing stability for another being true. Taking the minimum across them is a harsher rule than GRADE&rsquo;s, and a fund is entitled to say so.
        </P>
        <P>
          What survives is narrower and still enough. Separability argues against making the total the headline. It does not keep a published total unlabeled. The total collects a verb from whoever repeats it, and that verb will be the strongest one on the page.
        </P>
        <P>
          GRADE&rsquo;s other half answers the question a program officer raises next. Low certainty does not forbid action. GRADE allows a strong recommendation on low-certainty evidence in five defined situations, though it treats these as rare, and audits find guideline panels reach for them more often than the guidance sanctions (<a href="https://pubmed.ncbi.nlm.nih.gov/25618534/" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">Alexander et al., <I>J Clin Epidemiol</I>, 2016</a>). Certainty governs what a fund may claim. It does not govern what a fund may pay for. A pathway that earns only <I>supports</I> can still be worth funding, and both sentences can be said out loud without contradiction.
        </P>
        <P>
          It is worth naming what this is not. The responsible-AI frameworks a fund is likeliest to reach for do not close this gap either. The National Institute of Standards and Technology (NIST)&rsquo;s <a href="https://airc.nist.gov/airmf-resources/airmf/3-sec-characteristics/" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">AI Risk Management Framework</a> (2023) names validity and reliability as the first characteristic of a trustworthy system, and the <a href="https://eur-lex.europa.eu/eli/reg/2024/1689/oj" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">EU AI Act</a> (2024) sorts systems into risk tiers with obligations scaled to the tier. Neither treats validity in the argument-based sense meant here. For validation NIST adopts the ISO 9000:2015 definition, confirmation that the requirements for a specific intended use have been fulfilled, which asks whether a system performs as specified. NIST is revising that framework, and no revised version has been published. Argument-based validity asks whether a reported figure can carry the decision resting on it. The higher-education version of <InternalLink slug="what-is-this-system-measuring">the validity gate</InternalLink> is the same absence, surfacing wherever a number is asked to justify a decision.
        </P>
        ...
        <P>
          Three of those four columns are things a fund already has. The dollars come from the model it ran. Where the pathway caps comes from the four questions. The verb comes from the evidence sitting at that link. It asks for almost no new measurement. It asks a fund to show grading it already did.
        </P>
        <P>
        ...
        <P>
          GRADE has one more move worth borrowing, and it comes before the arithmetic. Not every outcome enters its summary rating. Only the ones a panel has judged critical to the decision count toward overall certainty; the rest are reported and set aside.
        </P>
        <P>
          Ported over, that asks a fund a question it does not currently ask. Which of these pathways is the claim actually about?
        </P>
        <P>
          If housing stability is central to what the fund is claiming, it counts, and it caps. If it is a secondary finding included because the data happened to exist, it does not belong in the aggregate. It can still be published. It stops setting the ceiling on a claim it was never central to. The declaration has to happen before the total is computed and it has to be visible, which is what keeps it from working as an escape hatch: a fund that reclassifies its weakest pathway after seeing the cap has done something a reader can see.
        </P>
        <P>
          There is a second effect, and for a small fund it may be the larger one. A pathway no decision depends on still consumes evaluation money and staff attention. Naming the critical pathways up front tells a fund where measurement effort is worth spending, and where it has been buying evidence for a claim it does not need to make.
        </P>
        <P>
        ...
        <MetaNote>
          Written August 2026 for the Analytic Bytes Library as a response to &ldquo;Dollars in Pockets: A New Way to Measure What Matters&rdquo; (NextLadder Ventures &amp; GitLab Foundation, July 2026; contributing authors Kyle Nelson, Tamsin Chen, Rhett Dornbach-Bender, and Ellie Bertani). The critique is offered in the spirit of the authors&rsquo; own invitation to apply the measure to the field&rsquo;s work and report where it breaks. Every description of Dollars in Pockets here is drawn from that published announcement. The complete methodology (the formulas, the discounting, the validation design) had not been published at the time of writing; the authors state their intention to publish it. Where this essay&rsquo;s argument depends on what that methodology does or does not report, the dependency is named in the body rather than assumed away. Revised 23 August 2026 to tighten attribution, separate NextLadder&rsquo;s fund-level ten-to-one target from the Rasa Legal worked example, and engage the authors&rsquo; stated position that ROI is one input among several. The same pass corrected the characterization of SROI, re-cited the Indian CSR impact-assessment requirement to Rule 8(3) of the CSR Policy Rules rather than to Section 135 of the Companies Act, separated the NIST AI RMF functions from the EU AI Act&rsquo;s risk tiers, and dated the Kane and Messick lineage. The same pass also split the fourth inference link, which had bundled a magnitude threshold with a contribution inference, and named the relationship between the four links, the three verb grades, and the Impact Management Project&rsquo;s five dimensions. The same pass named <I>contributed to</I> as the verb contribution analysis earns, and re-weighted the verb figure so that row &mdash; the one a fund is likeliest to be in, given what a controlled comparison costs &mdash; carries the emphasis rather than <I>produced</I>. The verb vocabulary is this essay&rsquo;s, not Mayne&rsquo;s. Mayne describes contribution analysis as assembling an argument, and does not prescribe a fixed set of verbs for its output. Revised 24 August 2026 to add the DALY&rsquo;s own contested history as the second half of the authors&rsquo; analogy, and to correct three citation errors found on a verification pass: the disability-weight critique is Nord (2013), the Messick citation is the 1995 <I>American Psychologist</I> article the link resolves to rather than the 1989 chapter, and the age-weighting and discounting parameters were contested for roughly two decades before the GBD 2010 revision, not a quarter century. A second pass the same day, prompted by two external critiques, made four structural changes. It separated the two things the minimum rule was being asked to do: the rule runs down a single pathway, where each inference depends on the one before it, and it does not license reading one weak pathway as evidence against a strong one beside it. It stopped implying that the minimum rule comes from Kane; argument-based validation motivates attention to the weakest assumption, and the arithmetic is this essay&rsquo;s operating choice. It added a closing section proposing that a fund publish per-pathway warrant beside the composite, which is the constructive form of the same argument. And it ordered the three verb rows by verb strength, since the previous order climbed to <I>produced</I> and then descended to <I>contributed to</I> while emphasizing the lower row. The same pass narrowed &ldquo;researched counterfactual,&rdquo; which had bucketed a matched comparison with a randomized one. The paragraph describing the Analytic Bytes youth-mental-health corpus was cut: the underlying longitudinal coding is not yet published, and an essay arguing that claims should not outrun inspectable evidence should not rest on evidence a reader cannot inspect. It will be restored with a cross-reference when that work publishes. Revised 25 August 2026 to add the GRADE precedent. Clinical guideline development has taken the lowest certainty rating among critical outcomes, rather than an average, for roughly two decades, which is independent support for the minimum rule this essay proposes. The same pass named the limit of that analogy &mdash; GRADE&rsquo;s outcomes are entangled where impact pathways are separable, so the rule is harsher here &mdash; and borrowed GRADE&rsquo;s separation of certainty from recommendation strength, which allows a fund to keep funding a pathway whose verb is weak. It also added the critical-pathway declaration, GRADE&rsquo;s practice of admitting only decision-critical outcomes into the summary rating, as a step a fund takes before computing a total. The verb ladder and the four links remain this essay&rsquo;s own; GRADE supplies precedent for the arithmetic, not the vocabulary. The NIST and EU AI Act passage was compressed in the same pass. A source-pack verification run on the same date corrected two GRADE citations that had been stated a notch stronger than their sources support: the summary-rating rule and its rationale now cite Guyatt et al. (2013) and Hultcrantz et al. (2017) with the word <I>generally</I> restored, and the sentence on strong recommendations from low-certainty evidence now carries GRADE&rsquo;s own censure of the practice alongside the audit finding that panels use it often. Seven source links were repointed to primary or stable locations or added. A second pass the same day removed the claim that the middle verb row is where most funders sit, in the body, the figure label and both alt strings: no survey supports it. The paragraph now reasons from what a controlled comparison costs, how long its effects take to compound, and the methodological expertise it asks for, to the expectation that many organizations will be in that row. The figure label reads WHERE COST PUTS MOST CLAIMS. The same pass corrected the description of contribution analysis. The essay had said its logic is &ldquo;elimination, not argument,&rdquo; which Mayne contradicts directly: he writes that contribution analysis provides an argument with evidence and builds a compelling case, a warrant, about the contribution being made. The elimination framing is this essay&rsquo;s inversion of his method for the purpose of grading a verb, and is now named as such. The word <I>warrant</I>, which this essay uses throughout, is Mayne&rsquo;s, and the lineage is now stated rather than assumed. The same pass narrowed &ldquo;additionality is the whole game,&rdquo; which the essay&rsquo;s own four-link structure contradicts: additionality is link four, and the minimum rule only matters because a weak link anywhere caps the claim. The Impact Management Project&rsquo;s dimensions were re-checked against Impact Frontiers on the same date and found correctly stated: five dimensions, with contribution fourth. The Indian Social Stock Exchange passage was brought current with SEBI&rsquo;s circular of 19 September 2025. The assessing body is an empanelled Social Impact Assessment Organisation rather than a registered Social Impact Assessor; the assessed report is required of enterprises that have raised funds on the exchange, and entities registered without a listed security may now self-report; a floor of 67 percent of prior-year program expenditure was added. The phrase &ldquo;on a logic-model frame&rdquo; was cut, since it could not be verified against the current circular. The self-certification carve-out is reported because it bears on this essay&rsquo;s argument: the amendment specifies more of the form while leaving the inference unaddressed. The operative regulation text was read in a secondary brief quoting it verbatim, as SEBI&rsquo;s attached PDF could not be opened directly. The substantive citation stays on the September 2025 amendment, which is where the change originated and the only text available; the January 2026 Master Circular is linked as the current consolidated framework, and its contents were not read directly. A further SEBI circular of 15 April 2026 partially modified the Master Circular, extending the period an NPO may stay registered without raising funds and lowering the minimum subscription threshold for Zero Coupon Zero Principal instruments. Neither change touches impact assessment, so the passage is unaffected. This is recorded so a later pass does not reopen it. The NIST passage was verified against the AI Resource Center on 25 August 2026. AI RMF 1.0 of January 2023 remains the only finalized core version; what NIST has issued since are profiles, including a Generative AI Profile in 2024 and an April 2026 concept note for a critical-infrastructure profile. NIST states on its own pages that the framework is under revision, which the essay now notes, since a reader may arrive after a revised version publishes. The validation definition was re-attributed: NIST quotes ISO 9000:2015 rather than defining the term itself. Section 3 of the framework also states that trustworthiness is only as strong as its weakest characteristic. That line was considered as a second precedent for the minimum rule and left out. NIST states the principle in prose and derives no arithmetic from it, so it is weaker than the GRADE precedent already cited, and placing it alongside GRADE would dilute rather than strengthen the case.
        </MetaNote>
      </>
    ),
  },
  ...
  // ===================================================================
  {
    ...
    body: (
      <>
        ...
        <MetaNote>
          <I>Method note.</I> Written August 2026 for the Analytic Bytes Library as the third piece in the measurement-validity arc, following What is this system actually measuring? (higher education) and The valid dollar (impact investing). The rubric material is drawn from Trovane&rsquo;s handbook for evaluating AI mental-health tools (v1.2, 2026), including its author-disclosure appendix, and from the published description of the governance layer underneath the Lumara platform, which is the company&rsquo;s own account and is cited as such. Matthew Krome, co-founder of Trovane, read a draft and contributed the governance-latency argument in the third question; the clock-start caution attached to it is mine. Trovane was known as Mental Health Chat until 13 August 2026; the handbook, the preview post, and the platform described here are the same work under the current name. Krome read the final draft and confirmed he is comfortable being named.
          <br /><br />
          <I>Sources, and how far each was read.</I> Four studies carry an argument here about how a number was produced, and for those an abstract is not enough, so each was read in full including its methods: McBain et al., JAMA Pediatrics, June 2026 (DOI 10.1001/jamapediatrics.2026.2015), a RAND-led nationally representative survey; Ibrahim et al., 2026 (arXiv 2605.07912); Obermeyer et al., Science, 2019; and Weilnhammer et al., Nature Medicine, 7 August 2026, whose Supplementary Information was read as well, including the Table 2 that holds the definitions of the thirteen rubric dimensions. In keeping with the second validity question this essay proposes: that paper&rsquo;s senior author is employed by Microsoft AI, its human annotation study was funded by Microsoft AI, and that study was conducted as part of Microsoft&rsquo;s standard product development activities and was not considered human subjects research, all disclosed in the paper. The VERA-MH human-validation study is cited as the February 2026 preprint, arXiv 2602.05088, which is the version this essay worked from and the version the Jester analysis responds to. The Bryan Jester analysis is a single secondary account and is attributed as such in the text rather than treated as an independent result. Other citations &mdash; Parente Health (July 2026), the NIMH RDoC framework (2010), the APA, NAM, WHO, Polaris, and ASL-MH documents &mdash; are used for their stated positions rather than for how a number was produced.
          <br /><br />
          <I>Post-publication record, consolidated 25 August 2026.</I> Published 14 August and corrected substantially since. Four changes matter enough to name &mdash; three where a reader who quoted the original would now be quoting something false, and one where a figure that circulated publicly has since been relabelled. Smaller fixes to framing and sourcing are kept in the working file.
          <br /><br />
          <I>Nature Medicine.</I> The essay originally said that none of the paper&rsquo;s checks validates the target. That was too strong. The known-groups test, where the judge separated high- from low-risk conversations at a median AUC of 0.98, is target-side evidence. But the Methods say where those conversations came from: a separate model, Grok-4, was given the written descriptions of the judge&rsquo;s own dimensions and asked to generate conversations matching them, so the stimulus was written from the rubric and then scored against it. Separately, the essay said the clinicians in the criterion check applied the judge&rsquo;s rubric. They did not. They rated each response on a single 1-to-10 slider, from safe and appropriate to clearly harmful.
          <br /><br />
          <I>The youth-use figures.</I> The demographic paragraph opened with &ldquo;roughly one in eight US adolescents.&rdquo; That is the same team&rsquo;s 2024 figure on different survey wording; the 2025 figure this paper reports is 19.2 percent. The Black-versus-White odds ratio of 5.45 ran without its interval, 1.44 to 20.66, and without its condition &mdash; it holds among young people who use a chatbot, not among all of them. The girls-twice-as-likely ratio belongs to any use, not to monthly use. And use was higher, not lower, among young people who had recently spoken with a physician, which contradicted the essay&rsquo;s earlier characterisation of these users as least served elsewhere. Two paragraphs were rebuilt around that finding.
          <br /><br />
          <I>Obermeyer.</I> The essay said changing the prediction target from cost to illness would have raised the share of Black patients flagged from 17.7 to 46.5 percent. Those figures belong to a different operation in that paper &mdash; a counterfactual patient swap that requires already knowing which patients are sick. The label change itself is Table 2, and it runs from 14.1 to 26.7 percent.
          <br /><br />
          <I>The six drift modes.</I> The second family was originally called the target family. It is now the claim family. That is not a relabelling. Target still names a property of the measurement, so both families ended up describing the instrument. Claim moves the second family out to interpretation and use, which is where Messick locates validity and where those three modes actually do their work &mdash; what the number means, who it applies to, and what it does to the people it is applied to. The first version of the figure circulated on LinkedIn under the earlier label, so anyone who responded to it was responding to the narrower version.
        </MetaNote>
      </>
    ),
  },
