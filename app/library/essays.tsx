  ...
  // ===================================================================
  {
    ...
    body: (
      <>
        ...
        <P>
          The Mental Health Chat handbook shows the pattern clearly, and it shows it because it is the most thorough of the set rather than the weakest. Its crisis-detection question asks whether detection accuracy has been audited across demographic groups: age, gender, First Nations status, users from culturally and linguistically diverse backgrounds, users with English as an additional language, neurodivergent users. That is a fairness question and a good one. It also assumes the construct is settled. A detector can only be tested for even performance across groups once there is an account of what it detects. None of the thirteen rows asks what construct the system is measuring, or what evidence supports the claim that it measures it. The closest any row comes is the audit question, whose top tier lists outcome tracking as two words alongside drift detection, with nothing behind it about which outcome or against what. What carries the safety claim across all thirteen is clinical veto: a named, credentialed clinician holds documented authority to block any product decision that conflicts with clinical safety. Their handbook draws a sharp line between clinically informed, meaning a clinician was consulted at some point, and clinically governed, meaning a named clinician holds ongoing accountability and can say no. The distinction is real, and most of the market fails it. Clinical veto establishes who is accountable for a decision. Evidence that the decision is measuring what it claims is a different thing, and no row asks for it.
        </P>
        <P>
          The same omission shows up in the strongest academic version of this work, which is the more telling case. In August 2026 a team from the Max Planck UCL Centre for Computational Psychiatry, Oxford, and the UK AI Security Institute published a framework in <I>Nature Medicine</I> for <a href="https://www.nature.com/articles/s41591-026-04577-2" target="_blank" rel="noopener" className="text-accent hover:text-accent-2 no-underline border-b border-line-2 hover:border-accent pb-px">auditing how AI chatbots behave in mental-health conversations</a>. A model plays a user with a psychiatric vulnerability, talks to a target chatbot for up to ten turns, and a judge model scores every turn against a thirty-nine-dimension rubric. They ran eight hundred and ten conversations across nine chatbots and produced more than ninety thousand turn-level ratings. The paper is titled a clinically validated framework and it earns much of that title. The validation work is the most serious in this literature: judge against judge, replicate against replicate, known-groups separation, stability when the rubric is paraphrased, a check for whether models grade their own output more leniently, and twenty-seven licensed clinicians producing four hundred and eighty-eight independent ratings.
        </P>
        <P>
          Every one of those checks validates the rater. None of them validates the target. The construct being scored is called concerning behavior, and the paper&rsquo;s stated reason for using it is its high face validity, which means it looks right to people who know the area. The criterion check asks whether the model agrees with clinicians applying the same rubric, which establishes that a definition is being applied consistently rather than that the definition names a real thing. Those clinicians agree with each other at an intraclass correlation of 0.31, which is poor by any standard, so the human criterion the model is validated against is itself unstable. The words construct validity, measurement, and psychometric do not appear in the paper.
        </P>
        <P>
        ...
        <Figure
          ...
          alt="A vertical stack of four numbered cards, Q1 through Q4, listing the validity questions the responsible-AI floor depends on: name the construct, produce validation evidence, name where the case is thinnest, and specify a drift-monitoring cadence."
        ...
        <Figure
          ...
          alt="Six drift modes in two families. The instrument family — model drift, conversational drift, detection-accuracy regression — is what responsible-AI monitoring already covers. The target family — construct drift, population drift, user-effect drift — is what the validity layer adds."
